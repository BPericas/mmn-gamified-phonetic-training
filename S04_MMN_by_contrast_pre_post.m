%% S04_MMN_by_contrast_pre_post.m
%
...
filePatterns = {'*_GEDAIclean.set', '*_fixed_GEDAIclean.set'};

% Get all files matching the desired patterns and split into pre and post
allFiles = [];
for p = 1:numel(filePatterns)
    allFiles = [allFiles; dir(fullfile(dataPath, filePatterns{p}))]; %#ok<AGROW>
end
...
for sess = 1:numel(sessions)
    ...
    files     = sessions(sess).files;    % Use the pre/post split computed earlier
    nFiles    = numel(files);
...
ylabel('Amplitude (\muV)', 'FontSize', 30, 'FontWeight', 'bold');
title('Grand-average MMN — Contrast 1 (beat/bit): Pre vs Post', 'FontSize', 30, 'FontWeight', 'bold');
grid on;
...
xlabel('Time (s)', 'FontSize', 30, 'FontWeight', 'bold');
ylabel('Amplitude (\muV)', 'FontSize', 30, 'FontWeight', 'bold');
title('Grand-average MMN — Contrast 2 (bat-bet): Pre vs Post', 'FontSize', 30, 'FontWeight', 'bold');
grid on;
